...
use crate::{central_state, constants::WEB3_NAME_SERVICE, utils::get_reverse_key};
use {
...
impl<'a, 'b: 'a> Accounts<'a, AccountInfo<'b>> {
    pub fn parse(
        ...
        // Check keys
        check_account_key(accounts.name_service_id, &WEB3_NAME_SERVICE)?;
        check_account_key(accounts.system_program, &system_program::ID)?;
        check_account_key(accounts.central_state, &central_state::KEY)?;

        // Check ownership
        check_account_owner(accounts.domain, &WEB3_NAME_SERVICE)
            .or_else(|_| check_account_owner(accounts.domain, program_id))?;
        check_account_owner(accounts.reverse, &WEB3_NAME_SERVICE)

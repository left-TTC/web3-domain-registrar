...
use crate::{
    central_state, constants::WEB3_NAME_SERVICE, cpi::Cpi, utils::{get_hashed_name, get_name_key}
};
...
impl<'a, 'b: 'a> Accounts<'a, AccountInfo<'b>> {
    pub fn parse(
        ...
        // Check keys
        check_account_key(accounts.naming_service_program, &WEB3_NAME_SERVICE).unwrap();
...
pub fn process_create_reverse(
    ...
    match (accounts.parent_name, accounts.parent_name_owner) {
        ...
        (Some(parent_name), Some(parent_name_owner)) => {
            check_account_owner(parent_name, &WEB3_NAME_SERVICE).unwrap();
